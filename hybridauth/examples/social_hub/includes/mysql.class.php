...
class MySQL
{
	...
	private $db_user    = "softoasi_referal";          // user name
	private $db_pass    = "]N^fwqZ*@7X9";          // password
	private $db_dbname  = "softoasi_referral";          // database name
	...
	*/
    ...
    public function getStatus($campaignID) {
        ...
        if (! $this->IsConnected()) {
            ...
            $sql = "SELECT startTime, endTime, refProgStatus FROM referralprogs WHERE refProgID='" . $campaignID . "'";
            $results = self::QueryArray($sql);
            // Return the result
            if (!$results) {
                return false;
            } else {
                ...
                //echo $date.'<br />';
                //echo $results[0]['startTime'].'<br />';
                //echo $results[0]['endTime'].'<br />';
                if ($results[0]['endTime'] < $date) {
                        return 1; //Expired.
                    } elseif ($results[0]['startTime'] > $date) {
                            return 2; //Not started yet.
                        } elseif ($results[0]['refProgStatus'] == 'Active') {
                                return 3; //Campaign live and active.
                        }else{
                                return 4; //within time defined but not active(Inactive, Deleted or Cancelled).
    ...
    public function getDates($campaignID) {
        $this->ResetError();
        if (! $this->IsConnected()) {
            $this->SetError("No connection");
            return false;
        } else {
            $sql = "SELECT startTime, endTime FROM referralprogs WHERE refProgID='" . $campaignID . "'";
            $results = self::QueryArray($sql);
            // Return the result
            if (!$results) {
                return false;
    ...
    public function getCampaignType($campaignID) {
        ...
        if (! $this->IsConnected()) {
            ...
            $sql = "SELECT refProgTypeID FROM referralprogs WHERE refProgID='" . $campaignID . "'";
            $result = self::QuerySingleValue($sql);
            // Return the result
            if (!$result) {
                return false;
            } else {
    ...
    public function getCampaignGoal($campaignID) {
        $this->ResetError();
        if (! $this->IsConnected()) {
            $this->SetError("No connection");
            return false;
        } else {
            $sql = "SELECT minReferralRequired FROM referralprogs WHERE refProgID='" . $campaignID . "'";
            $result = self::QuerySingleValue($sql);
            // Return the result
            if (!$result) {
                return false;
    ...
    public function getSocialShareReward($campaignID) {
        $this->ResetError();
        if (! $this->IsConnected()) {
            $this->SetError("No connection");
            return false;
        } else {
            $sql = $sql = "SELECT referralProgSocShareReward FROM  referralprogdetails WHERE referralProgID='" . $campaignID . "'";
            $result = self::QuerySingleValue($sql);
            // Return the result
            if (!$result) {
                return false;
    ...
    public function getTCs($campaignID) {
        $this->ResetError();
        if (! $this->IsConnected()) {
            $this->SetError("No connection");
            return false;
        } else {
            $sql = $sql = "SELECT referralProgTCs FROM  referralprogdetails WHERE referralProgID='" . $campaignID . "'";
            $result = self::QuerySingleValue($sql);
            // Return the result
            if (!$result) {
                return false;
    ...
    public function getImage($campaignID, $index = 0) {
        $this->ResetError();
        if (! $this->IsConnected()) {
            $this->SetError("No connection");
            return false;
        } else {
            $sql = "SELECT * FROM rp_referralprog_images WHERE referralProgID='" . $campaignID . "'  ORDER BY referralProgImageID Asc";
            $results = self::QueryArray($sql);
            // Return the result
            if (!$results) {
                return false;
            } else {
                return $results[$index]['referralProgImage'];
            }
        }
    }

    public function getDescription($campaignID) {
        $this->ResetError();
        if (! $this->IsConnected()) {
            $this->SetError("No connection");
            return false;
        } else {
            $sql = "SELECT referralProgDescription FROM  referralprogdetails WHERE referralProgID='" . $campaignID . "'";
            $results = self::QuerySingleValue($sql);
            // Return the result
            if (!$results) {
                return false;
            } else {
    ...
    public function getTitle($campaignID) {
        $this->ResetError();
        if (! $this->IsConnected()) {
            $this->SetError("No connection");
            return false;
        } else {
            $sql = "SELECT referralProgTitle FROM  referralprogdetails WHERE referralProgID='" . $campaignID . "'";
            $results = self::QuerySingleValue($sql);
            // Return the result
            if (!$results) {
                return false;
    ...
    public function getFromEmail($campaignID) {
        $this->ResetError();
        if (! $this->IsConnected()) {
            $this->SetError("No connection");
            return false;
        } else {
            $sql = "SELECT emailFromEmail FROM rp_email_templates WHERE refProgID='" . $campaignID . "'";
            $results = self::QuerySingleValue($sql);
            // Return the result
            if (!$results) {
                return false;
    ...
    public function getFromName($campaignID) {
        $this->ResetError();
        if (! $this->IsConnected()) {
            $this->SetError("No connection");
            return false;
        } else {
            $sql = "SELECT emailFromName FROM rp_email_templates WHERE refProgID='" . $campaignID . "'";
            $results = self::QuerySingleValue($sql);
            // Return the result
            if (!$results) {
                return false;
    ...
    public function getEmailMsg($campaignID, $type) {
        $this->ResetError();
        if (! $this->IsConnected()) {
            $this->SetError("No connection");
            return false;
        } else {
            $sql = "SELECT ETD.emailHtml FROM rp_email_template_details as ETD LEFT JOIN rp_email_templates as ET ON ET.emailTempID=ETD.emailTempID WHERE refProgID='" . $campaignID . "' AND ETD.type='".$type."'";
            $result = self::QuerySingleValue($sql);
            // Return the result
            if (!$result) {
                return false;
    ...
    public function getEmailSubject($campaignID, $type) {
        $this->ResetError();
        if (! $this->IsConnected()) {
            $this->SetError("No connection");
            return false;
        } else {
            $sql = "SELECT ETD.emailSubject FROM rp_email_template_details as ETD LEFT JOIN rp_email_templates as ET ON ET.emailTempID=ETD.emailTempID WHERE refProgID='" . $campaignID . "' AND ETD.type='".$type."'";
            $result = self::QuerySingleValue($sql);
            // Return the result
            if (!$result) {
                return false;
    ...
    public function addImpression($campaignID) {
        $this->ResetError();
        if (! $this->IsConnected()) {
            $this->SetError("No connection");
            return false;
        } else {
            $sql = "SELECT no_impressions FROM  referralprogs WHERE refProgID='" . $campaignID . "'";
            $impressions = self::QuerySingleValue($sql);
            $impressions += 1;
            //echo $impressions;
            $query = self::UpdateRows('referralprogs', array("no_impressions" => $impressions), array("refProgID" => $campaignID));
            // Return the value
            return $query;
        }
    }

    public function addClick($campaignID) {
        $this->ResetError();
        if (! $this->IsConnected()) {
            $this->SetError("No connection");
            return false;
        } else {
            $sql = "SELECT no_clicks FROM  referralprogs WHERE refProgID='" . $campaignID . "'";
            $clicks = self::QuerySingleValue($sql);
            $clicks += 1;
            //echo $impressions;
            $query = self::UpdateRows('referralprogs', array("no_clicks" => $clicks), array("refProgID" => $campaignID));
            // Return the value
            return $query;
        }
    }
    ...
    public function getReferrals($user) {
        if(is_string($user)){
            $sql = "SELECT UR.referredEmail, UR.referralStatus FROM rpusers as U INNER JOIN rp_users_referrals as UR ON UR.referredByUserID=U.userID WHERE U.userEmail='" . $user . "'";
        }else{
            $sql = "SELECT UR.referredEmail, UR.referralStatus FROM rpusers as U INNER JOIN rp_users_referrals as UR ON UR.referredByUserID=U.userID WHERE U.userID=" . $user;
          }
        $this->ResetError();
        if (! $this->IsConnected()) {
            $this->SetError("No connection");
            ...
            $result = self::QueryArray($sql);
            //var_dump($result);
            $aResult = array();
            //echo count($result);
            if($result){
                foreach($result as $referee){
                    $aResult[$referee[0]] = $referee[1];
                }
                return $aResult;
            }else{
                return $result;
    ...
    public function isRegistered($email) {
        ...
        if (! $this->IsConnected()) {
            ...
            $sql = "SELECT userID FROM rpusers WHERE userEmail='" . $email . "' AND userReferralPURL IS NOT NULL";
            $user = self::QuerySingleValue($sql);
    ...
    public function addUser($valuesArray) {
        ...
        if (! $this->IsConnected()) {
            ...
            $Id = self::InsertRow('rpusers', $valuesArray);
            return $Id;
        }
    }

    public function addReferee($valuesArray) {
        $this->ResetError();
        if (! $this->IsConnected()) {
            $this->SetError("No connection");
            return false;
        } else {
            $Id = self::InsertRow('rp_users_referrals', $valuesArray);
            return $Id;
        }
    }

    public function addInscription($referralID, $referee_email) {
        $this->ResetError();
        if (! $this->IsConnected()) {
            $this->SetError("No connection");
            return false;
        } else {
            $sql = "SELECT userEmail FROM rpusers WHERE userReferralPURL='" . $referralID . "'";
            $user_email      = self::QuerySingleValue($sql);
            $valuesArray    = array(
    ...
    public function getInscriptions($userEmail) {
        $this->ResetError();
        if (! $this->IsConnected()) {
            $this->SetError("No connection");
            return false;
        } else {
            $sql = "SELECT * FROM rpusers WHERE userEmail='" . $userEmail . "'";
            $this->Query($sql);
            return $this->RowCount();
        }
    }

    public function getDataForEmail($valuesArray) {
        $this->ResetError();
        if (! $this->IsConnected()) {
            $this->SetError("No connection");
            return false;
    ...
    public function getUserName($userID) {
        ...
        if (! $this->IsConnected()) {
            ...
            $sql = "SELECT userName FROM rpusers WHERE userID='" . $userID . "'";
            $user = self::QuerySingleValue($sql);
            return $user;
        }
    }
    ...
    public function getUserEmail($userID) {
        ...
        if (! $this->IsConnected()) {
            ...
            $sql = "SELECT userEmail FROM rpusers WHERE userID='" . $userID . "'";
            $user = self::QuerySingleValue($sql);
            return $user;
        }
    }

    public function getPurl($userID) {
        $this->ResetError();
        if (! $this->IsConnected()) {
            $this->SetError("No connection");
            return false;
        } else {
            $sql = "SELECT userReferralPURL FROM rpusers WHERE userID='" . $userID . "'";
            $user = self::QuerySingleValue($sql);
            return $user;
        }
    }

    public function getUserID($userReferralPURL) {
        $this->ResetError();
        if (! $this->IsConnected()) {
            $this->SetError("No connection");
            return false;
        } else {
            $sql = "SELECT userID FROM rpusers WHERE userReferralPURL='" . $userReferralPURL . "'";
            $user = self::QuerySingleValue($sql);
            return $user;
        }
    }
