 ...
 */
class Hybrid_Providers_Plaxo extends Hybrid_Provider_Model_OAuth1
{
	function initialize() 
	{
		parent::initialize();
		...
		$this->api->api_base_url      = 'http://www.plaxo.com/oauth';//'http://www.plaxo.com/pdata';
		$this->api->authorize_url     = 'http://www.plaxo.com/oauth/authorize';
		$this->api->request_token_url = 'http://www.plaxo.com/oauth/request';
		$this->api->access_token_url  = 'http://www.plaxo.com/oauth/activate';
	}
	...
	{
		$userId = $this->getCurrentUserId();
        //http://www.plaxo.com/pdata/contacts/@me/@all
		$parameters = array();
		//$parameters['format']	= 'json';
		//$parameters['count'] = 'max';
		
		$response = $this->api->get('http://www.plaxo.com/pdata/contacts/@me/@all', $parameters);
        ...
        foreach($parts as $v)
        {
            ...
            if (preg_match('/"displayName":"((?:[^"]*(?:(?<=\\\\)")?)*?)".*?"emails":\[{"value":"([^"]*)/si', $v, $matches))
            {
                ...
                $email = htmlspecialchars_decode($matches[2]);
            ...
            { // else generally not needed any more
                $name = htmlspecialchars_decode($matches[2]);
                $email = htmlspecialchars_decode($matches[1]);
